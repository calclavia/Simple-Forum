...
class ForumUser
{
	...
	function __construct($id, $username, $email, $con)
	{
		...
		{
			$result = mysql_query(
					"SELECT * FROM {$table_prefix}users WHERE ID={$this->id} LIMIT 1", $con);
	...
	public static function setUp($con)
	{
		global $table_prefix;
		mysql_query(
				"CREATE TABLE IF NOT EXISTS {$table_prefix}users (ID int NOT NULL, Moderate varchar(255), Unread varchar(255), Posts int, Signature TEXT)",
	...
	public function save($con)
	{
		...
		if ($this->id > 0)
		{
			...
			{
				$query = "INSERT INTO {$table_prefix}users (ID, Moderate, Unread, Posts, Signature) VALUES ({$this
						->id}, '" . mysql_real_escape_string(serialize($this->moderate)) . "', '"
						. mysql_real_escape_string(serialize($this->unreadPosts))
						. "', {$this->posts}, '" . mysql_real_escape_string($this->signature)
						. "')";
				mysql_query($query, $con)
						or die("Failed to create user data: " . mysql_error() . ", Q = " . $query);
				return true;
			} else
			{
				$query = "UPDATE {$table_prefix}users SET Moderate='" . serialize($this->moderate)
						. "', Unread='" . serialize($this->unreadPosts)
						. "', Posts={$this->posts}, Signature='"
						. mysql_real_escape_string($this->signature)
	...
	public function onCreatePost($post, $con)
	{
		...
		while ($row = mysql_fetch_array($result))
		{
			...
			$query = "UPDATE {$table_prefix}users SET Unread='" . serialize($unread)
					. "' WHERE ID={$row["ID"]} LIMIT 1";
			mysql_query($query, $con)
					or die("Failed to save other user data: " . mysql_error() . ", Q = " . $query);
		}
	...
	public function printProfile()
	{
		...
		<div class='forum_profile'>
			<img src='http://www.gravatar.com/avatar/" . md5($this->email)
				. "?d=mm&s=160' alt='Avatar' class='avatar'/>
